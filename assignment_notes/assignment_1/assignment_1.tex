...
The environment is a custom-built **Grid World**, which is a discrete 5x3 grid ($5$ columns, $3$ rows). The agent's objective is to navigate from a starting state to a target state while avoiding forbidden zones. The specific configuration is as follows:
\begin{itemize}
    \item \textbf{Grid Size:} (Width, Height) = (5, 3)
    \item \textbf{Starting State:} (0, 2)
    \item \textbf{Target State:} (2, 2)
    \item \textbf{Forbidden States:} [(2, 1), (1, 2), (3, 1)]
\end{itemize}
...
The **discount rate**, $\gamma$, is set to **0.9**. This value determines the present value of future rewards, encouraging the agent to reach the target sooner rather than later. The discounted return is calculated as $G_t = \sum_{k=0}^{T-1} \gamma^k R_{t+k+1}$.
...
Starting from state \textbf{(0, 2)}, the agent's trajectory over 50 steps is simulated. The policy is visualized with green arrows on the grid, and the agent's path is shown as a green line.
...
\subsection{Policy Plot and Trajectory}
Again starting from state \textbf{(0, 2)}, the agent's trajectory is simulated. The policy is visualized with arrows whose thickness can represent probability, though here they show the possible actions.
...
In conclusion, a policy that intelligently uses the structure of the environment (like the BFS-based deterministic policy) is far superior to a policy that acts randomly without regard to consequences. The stochastic policy's failure illustrates a common challenge in reinforcement learning: an agent with a poor policy can easily get stuck in parts of the state space that yield low or negative rewards.
